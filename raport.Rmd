...
Na powyższym wykresie korelacji należy zwrócić uwagę na korelacje z długością śledzi. Najwyższa korelacja zauważalna jest z wartościami zagęszczenie Calanus helgolandicus gat. 1 (chel1), zagęszczenie widłonogów gat. 1 (lcop1), ułamek pozostawionego narybku (fbar), natomiast długość śledzia jest najmniej skorelowana z temperaturą przy powierzchni wody (sst) oraz oscylacją północnoatlantycką (nao).
...
W celu przedstawienia atrybutów, które, na podstawie dostępnych danych, są przyczyną karłowacenia śledzi wykorzystany zostanie model RandomForest. Dane zostały wyczyszczony i uruchomione na modelu RandomForest.

```{r, echo=FALSE}
plot(varImp(rf))
```

Na podstawie powyższego wykresu można stwierdzić, że największy wpływ ma temperatura wody oraz zagęszczenie widłogonów gat. 1. Atrybuty takie jak miesiąc oraz X, czlyi numer porządkowy, zostały pominięte celowo, ponieważ nie mają znaczenia dla naszej analizy danych.
